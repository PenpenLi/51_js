local WeaponExchangeResultPanel=class("WeaponExchangeResultPanel",UILayer)


function WeaponExchangeResultPanel:ctor(data1,data2)

    self.appearType = 1;
    self:init("ui/ui_weapon_transmit_result_mowen.map") 
    self:setCardData(data1, 1)
    self:setCardData(data2, 2)
    self.hideMainLayerInfo = true; 
    self.isWindow = true;
end

function WeaponExchangeResultPanel:getAttrFlag(attr)
    return CardPro.cardPros["attr"..attr]
end

 

function  WeaponExchangeResultPanel:setCardData(card,pos)
...
function WeaponExchangeResultPanel:onTouchEnded(target)
...
return WeaponExchangeResultPanel
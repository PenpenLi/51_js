...
function classCacheFunc(classname, super,funcs)
    ...
    if superType == "function" or (super and super.__ctype == 1) then
        ...
        function cls.new(...)
            ...
            for key, var in pairs(funcs) do
                instance["_"..var]=instance[var] 
            end
...
function removeBom(content) 
    if string.byte(content,1,3)==239 then 
        return string.sub(content,4, string.len(content) )  
    end
    return content
end
...
function table.valueByIdx(t,idx)
    local i=0
    for k, v in pairs(t) do
        if(i==idx)then
...
function string.splitUtf8ToTable(str)
    local ret = {};
    local index = 1;
    while(true) do
        local word = string.utf8sub(str,index,index);
        if(word == str or string.utf8len(word) == 0) then
            break;
        end
        -- print("spit word = "..word);
...
function string.filter(str)
    -- body
    for i,var in pairs(string.filterStr) do 
...
function print_lua_table (lua_table, indent)
    if(gPrintLuaTable == false)then
        return;
    end
    -- print("type = " .. type(lua_table));
    -- print(">>>>>>>>>>> start print_lua_table");
    if lua_table == nil or type(lua_table) ~= "table" then
        return
    end

    local function print_func(str)
        print("[Dgr LUA] " .. tostring(str))
    end
    indent = indent or 0
...
function math.disruptTable(srcTable)
    if nil == srcTable or #srcTable <= 1 then
        return nil
    end
    ...
    local idx = 0
    while count > 1 do
        idx = math.random(1, #srcTable)
        table.insert(dstTab, srcTable[idx])
        table.insert(dstIdxTab,srcIdxTab[idx])
        table.remove(srcTable, idx)
        table.remove(srcIdxTab,idx)
        count = #srcTable
    end
    table.insert(dstTab, srcTable[1])
    table.insert(dstIdxTab, srcIdxTab[1])
    return dstTab,dstIdxTab
end

--从dataCount中取出getCount个
function getRandomArray(getCount,dataCount)

  local ret = {};
  if dataCount < getCount then
    ...
    for i = 1,getCount do
        -- print("randarray");
        -- print_lua_table(indexs);
      local idx = math.random(1,table.count(indexs));
      -- print("insert index = "..indexs[idx]);
      table.insert(ret,indexs[idx]);
      table.remove(indexs,idx);
    end
  end

...
--[[
格式：
    文字:\\w{c=ffffff;s=20;f=xxx;o=ffffff00}wordContent
    or   \\wordContent  字体配置使用setDefaultConfig
    图片:\\i{p=xxxxx;s=0.5;}
例子：
    local rtf = RTFLayer.new(110);
...
function RTFLayer:layout()
    ...
    self:setContentSize(self.richText:getContentSize());
    self:ignoreAnchorPointForPosition(false);
    -- print("w = "..self:getContentSize().width .. " h = "..self:getContentSize().height);
end
...
function RTFLayer:decodeOneWord( str )
    ...
    elseif string.sub(str,1,2) == "c{" then
        --c{n=90001;s=0.5;v=0}
        --n=itemid;s=缩放比例;v=0-隐藏底框
        local pos = string.find(str,"}",2);
        if(pos ~= nil)then
            ret.type = "node";
            local config = string.sub(str,3,pos-1);
            ...
            for i,var in pairs(pros) do
                ...
                if first == "n" then
                    if string.len(var) >= 3 then
                        local itemid = string.sub(var,3);
                        -- print("itemid = "..itemid);
                        local node = cc.Sprite:create("images/ui_public1/ka_d6.png");
                        local icon,type = Icon.setIcon(toint(itemid),node,DB.getItemQuality(toint(itemid)));
                        ret.node = node;
                    end
                elseif first == "s" then
                    if string.len(var) >= 3 then
            ...
            if ret.node then
                if ret.scale then
                    ret.node:setScale(ret.scale);
                end
                if ret.hidebg and ret.hidebg == "0" then
                    local icon = ret.node:getChildByTag(1);
                    if icon ~= nil then
                        ret.node:removeChildByTag(1);
                        local node = cc.Node:create();
                        node:setContentSize(icon:getContentSize());
                        gAddChildInCenterPos(node,icon);
                        node:setScale(ret.node:getScale());
                        ret.node = node;
                    end
                    -- ret.node:setCascadeOpacityEnabled(false);
                    -- ret.node:setOpacity(0);
                end
            end
        end
...
function RTFLayer:addImage(imagePath,scale,opacity,color,tag)
    ...
    rc:setScale(scale);
    -- rc:setAnchorPoint(cc.p(0,1));
    self:addNode(rc,opacity,color,tag);
    -- local rc =ccui.RichElementImage:create(tag, color, opacity, imagePath);
    -- rc:setScale(scale);
    -- self.richText:pushBackElement(rc)
